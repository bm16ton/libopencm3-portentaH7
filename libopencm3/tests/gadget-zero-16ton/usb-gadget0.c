...
static void gadget0_ss_in_cb(usbd_device *usbd_dev, uint8_t ep)
{
    ...
    uint16_t x = usbd_ep_write_packet(usbd_dev, ep,&samples, sizeof(samples));
...
static void send_samples(uint8_t ep)
{
    ...
    if (!is_configured)
        return;
    usbd_ep_write_packet(our_dev, ep, &samples, sizeof(samples));
//     usbd_ep_write_packet(usbd_dev, 0x80 | ep, buf, x);
}
