...
//     struct irq_desc*  irq_descs    [5]; // IRQ descriptors used (irq_num elements)
	...
	struct spi_controller *spimaster;   //todo keep this for compatability but add controller
	...
	u8 tx_cmd;
	u8 xfer_buf[SZ_64K];
	u16 last_mode;   //used to compare old/new spi modes if diff update ciontroller
