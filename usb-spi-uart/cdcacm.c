...
void uart_dma_start(void *txbuf, size_t data_size);
...
RCC_BDCR = ( RCC_BDCR_RTCSEL_MASK << RCC_BDCR_RTCSEL_SHIFT | RCC_BDCR_RTCSEL_HSE);
printf("baud = %ld\r\n", coding->dwDTERate);
...
uint8_t buf[CDCACM_PACKET_SIZE] = {0};
	...
	uart_dma_start(buf, len);
//	if (len) {
//		while (usbd_ep_write_packet(usbd_dev, 0x85, buf, len) == 0);
//	}
...
nvic_enable_irq(NVIC_USART1_IRQ);
    ...
    nvic_set_priority(NVIC_DMA1_STR3_IRQ, 2);
    dma_stream_reset(DMA1, DMA_STREAM2);
    dma_set_priority_level(DMA1, DMA_STREAM2, DMA_SxCR_PL_HIGH);
	...
	dma_set_fifo_threshold(DMA1, DMA_STREAM2, DMA_THRESHOLD_QUARTER);
...
// RX DMA
    ...
    dma_disable_mburst(DMA1, DMA_STREAM3);
    dma_enable_pburst(DMA1, DMA_STREAM3);
	...
	dma_set_memory_address(DMA1, DMA_STREAM3, (uint32_t)0x0000);
	dma_set_number_of_data(DMA1, DMA_STREAM3, (uint16_t)0x00);
...
if (g_usbd_is_connected) {
while (usbd_ep_write_packet(g_usbd_dev, 0x85, buf, i) == 0);
    }
i = 0;
}
...
usart_disable(USART1);
	dma_set_memory_address(DMA1, DMA_STREAM2, (uint32_t) txbuf);
...
static void InitLdoAndPll(void) {
  ...
  static const struct rcc_pll_config pll_config = {
    ...
    .pll2 = {
        ...
        .divq = 8U,    // PLL1Q post-divider gives 80MHz output for FDCAN.
...
//	SCB_CleanDCache();
    ...
    usart_setup();
    sdramsetup();
    ramtest();   // fails without dcache, one write = one read
    SCB_DisableDCache();
...
for (unsigned p = 0; p < 30000; p++)
	  {
		__asm__("nop");
	  }
twaitm = (44);
//st_draw_bitmap(1, 110, &bm16ton);
for (unsigned p = 0; p < 130000; p++)
	  {
		__asm__("nop");
	  }
