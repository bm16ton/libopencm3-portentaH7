...
usbd_ep_nak_set(usbd_dev, ep, 1);
__DSB();
	len = usbd_ep_read_packet(usbd_dev, ep, buf, CDCACM_PACKET_SIZE);
	if (len > 0) {
...
volatile uint32_t poopsielst2 = 0;
volatile uint32_t poopsiettl = 0;
uint8_t b4usb[768] = {0};
volatile uint32_t oldpoopsie = 0;
volatile uint32_t currentold = 0;
