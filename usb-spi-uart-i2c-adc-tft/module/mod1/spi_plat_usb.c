...
/*
static void
devtree_probe (struct platform_device *pdev) {
    struct usb_spi_platform_data *pd = dev_get_platdata(&pdev->dev);
...
//	pd->spi_info->irq = GPIO_irqNumber;
    if (!device_property_present(dev, "cs-gpios")) {
		printk("cs-gpios property not found \n");
		priv->spiinfo.chip_select = 1;
		} else {
    printk("cs-gpios property found \n");
    	device_property_read_u32(dev, "cs-gpios", &num_cs);
    	priv->spiinfo.chip_select = num_cs;
//	    pd->spi_info[1].chip_select = cspin;
		}
printk ("failed 5\n");
    if (!device_property_present(dev, "compatible")) {
		printk("compatible/modalias property not found \n");
		strcpy(priv->spiinfo.modalias, "spi-petra");
		} else {
		printk("compatible/modalias property found \n");
    	device_property_read_string(dev, "compatible", &malias);
	    strcpy(priv->spiinfo.modalias, malias);
		}
printk ("failed 6\n");
...
//	  }
	  ...
	  sdev = spi_new_device (master, &priv->spiinfo);
	  if (sdev) {
	  ;
	  }
printk ("failed 7\n");
...
//if (device_property_present(dev, "fromdt")) {
...
//	    of_property_read_u32(
	    priv->spiinfo.chip_select = device_property_read_u8(dev, "css", &cspins);
	    printk("cs-gpio is %d", cspins);
	    device_property_read_string(dev, "compatible", &malias);
	    printk("modalias is %s", malias);
	    strcpy(priv->spiinfo.modalias, malias);
	    sdev = spi_new_device (master, &priv->spiinfo);
 //       devtree_probe(pdev);
        goto exit;
...
static const struct of_device_id usbspi_of_match[] = {
	{.compatible = "spi_plat_usb"},
	{},
};

MODULE_DEVICE_TABLE (of, usbspi_of_match);

static const struct spi_device_id usb_spi_ids[] = {
	{.name = "spiplatusb", (unsigned long) usbspi_probe},
	{},
};

MODULE_DEVICE_TABLE (spi, usb_spi_ids);

static const struct platform_device_id usb_plat_ids[] = {
	{"spi_plat_usb", (unsigned long) usbspi_probe},
