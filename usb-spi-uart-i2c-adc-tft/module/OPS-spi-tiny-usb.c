...
const char *gpio_names[] = { "CSpi1", "ce", "csn", "PJ11", "IRQpk1" };
...
static int usb_write2(struct usb_interface *, int cmd, int value, int index, void *data, int len);
...
usb_write(priv, cmd, value, index, data, len);
return 0;
}
...
//    printk("bens priv->gbase = %d\n", priv->gbase);   // FUNNY BOTH priv->chip.base AND THE CP'D priv->gpiobase ARE SET AND RETURN FINE I THINK SWITCHING INTERFACES
    ...
    setupe_data lock,(usbspi_lock); 
    csset(.setupe_data, priv->spidev, true);
...
static int usbspi_intf_spi_probe(struct usb_interface *intf,
			      ...
			      const struct usb_device_id *id)
{
   ...
   priv->chip.label = "usbspigpio"; //name for diagnostics
